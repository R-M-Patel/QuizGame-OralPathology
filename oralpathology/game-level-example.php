...
<script language="javascript">
	...
	$( document ).ready(function() {
    	$("input[class='imageSelector']").each(function (index, element) {
            $( this ).click(function() {
            	selImageID = element.id.replace('btn', ''); //set button to image
				isAnswerCorrect = checkIfCorrect(selImageID);
				if(isAnswerCorrect){ //if answer was correct
					$( this ).css({"background-color": "#00ff00"});
					updateScoreData('1243',selImageID, true);
				}
				else{ //if answer was wrong, alter
	...
	function updateScoreData(userID, selectedImageID, isCorrect){
		...
		console.log(JSON.stringify(scoreData)); //convert score to json and log it to console
		
		$.post( "updatescore.php", scoreData) //send the score to updatescore.php
		.done(function( data ) {
			console.log( "Data Loaded: " + data ); //log data
		});
	}	
...
for($i=0; $i<count($imageList); $i++){ //iterate through images
