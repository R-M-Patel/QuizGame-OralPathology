...
<script language="javascript" type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $("input[class='imageSelector']").each(function (index, element) { 
            $(this).click(function () { //function for if element is clicked
                var selImageID = element.id.replace('btn', ''); //set a button
                var questionID = imageList[0].questionID;
                var isAnswerCorrect = checkIfCorrect(selImageID); //check if image is correct
                if (isAnswerCorrect) { //mark as correct
                    ...
                    for (var i = 0; i < imageList.length; i++) { //iterate through images
                        if (imageList[i].imageID == selImageID) { 
                            correctAns = ("This image actually represents <br>" + imageList[i].diagnosis); //display image's diagnosis
                            document.getElementById("dialog-confirm").innerHTML = correctAns; //display correct answer
                            $(function () {
