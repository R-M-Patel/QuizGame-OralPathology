...
<script language="javascript" type="text/javascript">
        ...
        $("input[class='imageSelector']").each(function (index, element) {
            $(this).click(function () {
                ...
                if (isAnswerCorrect) {
                    ...
                    for (var i = 0; i < imageList.length; i++) { //iterate through image list
                        if (imageList[i].imageID == selImageID) { //if current index id matches selected image id
                            correctAns = ("This image actually represents <br>" + imageList[i].diagnosis); //display image diagnosis
	...
	/*formula for deducting points for hints */
  ...
  <div class="answers-div">
    ...
    for ($i = 0; $i < count($imageList); $i++) {
        if($i==0 || $i==2){
          ...
          if ($imageList[$i]->isCorr          if ($imageList[$i]->isCorrectChoice() == 1 && isset($_SESSION["debug"])) {
ectChoice() == 1 && isset($_SESSION["debug"])) {
              echo("<br />correct<br />");
          }
          echo("</div>");
        if($i==1 || $i==3){
