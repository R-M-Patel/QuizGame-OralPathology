...
<script language="javascript" type="text/javascript">
    ...
    function saveSelectedPairs() {
        if (tempSelectedDiagnosis != "" && tempSelectedImage != "") {
            ...
            calculateScore();
            getScoreData('<?php echo $_SESSION["gameAttemptID"]; ?>', '<?php echo $drupalUserID; ?>', 3); //display score data
        }
    }
  ...
  <div  class="row">
    <div class="col-md-2 terms">
      ...
      for ($i = 0; $i < count($questionList); $i++) {
          ...
          echo("<input type='button'" . "id='" . $q->getQuestionID() . "_" . $imgList[0]->getImageID() . "' class='btnTerm'" . "value='" . $imgList[0]->getAssociatedDiagnosis() . "' onclick='selectDiagnosisMatch(this);'/>");
      }
    ...
    <div class="col-md-10">
      ...
      for ($i = 0; $i < count($questionList); $i++) {
          ...
          if (isset($q)) {
              $imgList = $q->getImageList();
              if (isset($imgList)) {
                  echo("<div id='div" . $imgList[0]->getImageID() . "' class='imageContainer lvl3'>");
